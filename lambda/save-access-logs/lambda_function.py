    ...
    for item in response['Contents']:
        ...
        with gzip.GzipFile(fileobj=BytesIO(obj['Body'].read())) as gzipfile:
            ...
            for record in log_data['Records']:
                ...
                if record['eventName'] == 'PutObject':
                ...
                elif record['eventName'] == 'GetObject' and not('response-content-disposition' not in record['requestParameters']):
                    ...
                    try:
                        ...
                        if item:
                            ...
                            # event_time 업데이트와 count 필드 업데이트
                            table.update_item(
                                Key={'entity_tag_value': entity_tag_value},
                                UpdateExpression='SET event_time = :val1, #count_field = :count_value',
                                ExpressionAttributeNames={'#count_field': 'count'},
                                ExpressionAttributeValues={':val1': event_time, ':count_value': new_count}
                            )
                        else:
                            # 아이템이 없으면 새로운 아이템 생성
                            table.put_item(
